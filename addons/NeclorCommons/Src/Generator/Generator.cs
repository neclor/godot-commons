...
public class InterfaceGenerator : ISourceGenerator {
	...
	public void Execute(GeneratorExecutionContext context) {
		...
		foreach (var partialClass in GetPartialClasses(context)) {
			...
			List<(INamedTypeSymbol, MethodDeclarationSyntax, IMethodSymbol)> methods = GetMissingInterfaceMethods(classSymbol);
	...
	static List<(INamedTypeSymbol Interface, MethodDeclarationSyntax Syntax, IMethodSymbol Symbol)> GetMissingInterfaceMethods(INamedTypeSymbol classSymbol) {
	...
	static string GenerateMethodCode(INamedTypeSymbol interfaceSymbol, MethodDeclarationSyntax methodSyntax, IMethodSymbol methodSymbol) {
		StringBuilder stringBuilder = new();
		stringBuilder.AppendLine("");

		string returnType = ыymbol.ReturnType.ToDisplayString();
		...
		stringBuilder.AppendLine($"(({interfaceSymbol.ToDisplayString()})this).{methodSymbol.Name}       ");
