...
func generate(size: Vector2i = room_size) -> Grid:
	...
	var central_rect_position: Vector2i = 
	var grid_central_rect_size: Vector2i = central_rect_size * cell_size
	var grid_central_rect_position: Vector2i = grid.center - grid_central_rect_size / 2 # сначала делить потом умножать
	prints(central_rect_size)
	grid.fill_rect(grid_central_rect_position, grid_central_rect_size, grid.Cell.FILLED)
	...
	for i in extra_rect_count: 
		...
		prints(rect_size, rect_offset)
		var grid_rect_size: Vector2i = rect_size * cell_size
		var grid_rect_position: Vector2i = grid.center - grid_rect_size / 2 + rect_offset * cell_size

		grid.fill_rect(grid_rect_position, grid_rect_size, grid.Cell.FILLED)
